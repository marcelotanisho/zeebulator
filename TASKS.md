...
- [ ] MIF (Module Information File) parser — **partially understood, not
      implemented.** No byte-level spec exists publicly either (confirmed
      via web research). What's confirmed: MIF is generated by compiling
      CIF (Component Information File) sources with Qualcomm's `cifc.exe`;
      contains a resource table + a UTF-16LE string table (each string
      prefixed with an `0xFFFE` BOM marker, confirmed by directly finding
      the plaintext app name `"Dragon Vs Chicken"` at a header-referenced
      offset in Double Dragon's `.mif`). A real, sourced lead for the
      resource table's meaning: `AEEShell.h`'s resource-ID offset scheme
      (`IDR_NAME_OFFSET=0, IDR_ICON_OFFSET=1, IDR_IMAGE_OFFSET=2,
      IDR_THUMB_OFFSET=3, IDR_SETTINGS_OFFSET=4, IDR_VERSION_OFFSET=5,
      IDR_ENVIRONMENT_OFFSET=6, IDR_OFFSET_STEP=20`) — untested against
      real files yet. Also noted: at least one known BREW/Zeebo title's
      MIF is reportedly encrypted (per a Tuxality devlog), so not every
      MIF may be plain-structured. Next step: apply the same
      hypothesis-and-cross-test method used successfully on GGZ.
...
- [ ] Test fixtures: **deviated from the original plan.** SDK sample apps
      don't include any `.ggz` files at all (only Double Dragon's
      commercial dump does), so GGZ tests use self-generated synthetic
      archives instead (`tests/ggz_test.cpp`) — real-file correctness was
      confirmed manually via `zeebulator_ggz_inspector` against the local,
      git-ignored Double Dragon dump rather than as a committed automated
      test. Revisit for MIF/.mod once those are implemented — SDK sample
      `.mif` files *are* available and could become real committed
      fixtures if their content isn't legally sensitive (they're official
      Qualcomm SDK samples, not commercial game data).
